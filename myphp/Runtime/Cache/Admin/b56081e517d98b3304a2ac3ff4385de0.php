...
<HEAD>
    ...
    <SCRIPT src="/Public/js/jQuery.easyui.js" type="text/javascript"></SCRIPT>
	<SCRIPT src="/Public/js/jquery.easyui.min.js" type="text/javascript"></SCRIPT>
	<!-- [EasyUI] -->
	<link rel="stylesheet" type="text/css" href="/Public/js/themes/default/easyui.css" />
	<link rel="stylesheet" type="text/css" href="/Public/js/themes/icon.css" />
	...
	<STYLE>
		...
		* {
		...
		.ma{ width:100px !important; margin-top:10px; margin-left:33px;}
		.yzm{ width:100px; height:28px; display:inline-block; top:15px; position:absolute; margin-left:15px;}
	...
	<SCRIPT type="text/javascript">
		...
		function submitForm(){
			...
			$.post('<?php echo U('Admin/Login/login');?>', $("#loginform").serialize(), function(data){
                console.log(data);
				if(!data.status){
...
<DIV style="background: rgb(255, 255, 255); margin: -100px auto auto; border: 1px solid rgb(231, 231, 231); border-image: none; width: 400px; height: 245px; text-align: center;">
	...
	<P style="padding: 30px 0px 10px; position: relative;"><SPAN
			class="u_logo"></SPAN>
		<INPUT class="ipt" type="text" placeholder="请输入用户名或邮箱" name="username" id ="username">
	</P>
	...
	<P style="position: relative; text-align:left;">
		<INPUT class="ipt ma" id="validate" type="text" placeholder="请输入验证码" name="validate">
		<span class="yzm"><img id="code_img" align="top" onclick="changeCode()" src="code?code_len=4&font_size=14&width=100&height=28&code="+time() title="点击切换验证码"></span>
	</P>
		...
		<P style="margin: 0px 35px 20px 45px;"><SPAN style="float: left;"><A style="color: rgb(204, 204, 204);"
																			 href="#">忘记密码?</A></SPAN> <SPAN style="float: right;"><A style="color: rgb(204, 204, 204); margin-right: 10px;"
																																	  href="#">注册</A> <A style="background: rgb(0, 142, 173); padding: 7px 10px; border-radius: 4px; border: 1px solid rgb(26, 117, 152); border-image: none; color: rgb(255, 255, 255); font-weight: bold;"
																																						 href="javascript:;" onclick="submitForm()">登录</A> </SPAN> </P>
	</DIV>
		</form>
</DIV>
